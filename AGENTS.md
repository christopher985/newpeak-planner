...
- Codex는 사용자가 수정사항을 요청했고 실제 파일 변경을 반영한 경우, 검증 가능한 최소 검사를 수행한 뒤 로컬 Git 커밋까지 자동으로 생성합니다.
- 단순 조사, 설명, 리뷰, 계획 수립처럼 파일 변경이 없는 작업은 커밋하지 않습니다.
- 사용자가 "커밋하지 마", "수정만 해", "확인만 해"처럼 커밋 보류를 명시하면 자동 커밋하지 않습니다.
- 자동 커밋 전에는 작업트리를 확인하고, 현재 요청 범위에 속한 파일만 stage합니다. 무관한 사용자 변경이 섞여 있으면 해당 변경은 stage하지 않고 보고합니다.
- 자동 커밋 메시지는 변경 의도를 짧게 요약하는 영어 문장형 제목을 사용합니다.
- 자동 커밋 후에는 커밋 해시, 포함 파일, 검증 결과를 최종 보고합니다.
- push 또는 `..\push.bat` 실행 전에는 변경 내용, 영향 범위, Netlify 배포 발생 가능성을 먼저 보고합니다.
- push는 Netlify 자동 배포를 유발하므로, 사용자가 명시적으로 "푸시", "배포", "`push.bat` 실행"을 요청한 경우에만 수행합니다.
